<div class=" mx-auto w-full">
  ...
  <div class="h-64 mx-auto mb-5 relative">
    <div class="absolute bg-black opacity-40 w-full h-full"></div>
    <div class="absolute top-0 right-0 left-0 bottom-0 flex flex-col items-center justify-between">
      <p class="p-1 my-2"></p>
      <p class="py-3 px-1 text-white font-medium text-2xl rounded">{{$company->name}}</p>
      <p class="py-3 px-1 text-white font-medium text-2xl rounded">{{$company->description}}</p>
    </div>
    <img src="{{asset('storage/portada/'.$company->image)}}" alt="imagen portada" class="w-full h-full object-cover rounded-sm" />
  ...
  <div class="px-1">
      <x-sistem.accordions.accordion-1 :categories="$categories"/>
  </div>

  <div>
    {{-- <x-sistem.carousels.carousel-1/> --}}
    <div class="w-full h-32 bg-purple-800 text-white flex items-center justify-center text-2xl my-3">
        <p>Productos destacados</p>
    </div>
    @foreach ($suggesteds as $suggested)
        <div class="h-64 mx-auto mb-5 relative">
            <div class="absolute bg-black opacity-40 w-full h-full"></div>
            <div class="absolute left-0 bottom-0 flex flex-col items-left justify-start">
              <p class="py-3 px-1 text-white font-medium text-lg rounded">{{$suggested->product->name}}</p>
              <p class="px-1 text-white font-medium text-sm rounded">{{$suggested->product->description}}</p>
            </div>
            <img src="{{asset('storage/portada/'.$suggested->product->image)}}" alt="imagen portada" class="w-full h-full object-cover rounded-sm" />
        </div>
    @endforeach
  </div>
    ...
    <div
        ...
        <div class="flex flex-wrap flex-grow mt-10 -mb-10 text-center md:pl-20 md:mt-0 md:text-left">
            <div class="w-full px-4 lg:w-1/4 md:w-1/2">
                <h2 class="mb-3 text-sm font-medium tracking-widest text-gray-900 uppercase title-font">{{$company->name}}</h2>
                <nav class="mb-10 list-none">
                    <li class="mt-3">
                        <a class="text-gray-500 cursor-pointer hover:text-gray-900">{{$company->email}}</a>
                    </li>
                    <li class="mt-3">
                        <a class="text-gray-500 cursor-pointer hover:text-gray-900">{{$company->phone}}</a>
                    </li>
                    <li class="mt-3">
                        <a class="text-gray-500 cursor-pointer hover:text-gray-900">{{$company->adress}}</a>
                    </li>
                </nav>
            </div>
            <div class="w-full px-4 lg:w-1/4 md:w-1/2">
                <h2 class="mb-3 text-sm font-medium tracking-widest text-gray-900 uppercase title-font">Acceso</h2>
                <nav class="mb-10 list-none">
                    <li class="mt-3">
                        <a href="{{route('login')}}" class="text-gray-500 cursor-pointer hover:text-gray-900">Administrar</a>
                    </li>
                </nav>
            </div>
        </div>
