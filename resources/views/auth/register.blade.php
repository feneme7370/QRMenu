          ...
          <div class="flex flex-col overflow-y-auto md:flex-row">
            <div class="h-32 md:h-auto md:w-1/2">
              <img
                ...
                src="{{asset('lib/windmill/img/create-account-office.jpeg')}}"
                alt="Office"
              />
            </div>
            <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
              <div class="w-full">
                <form method="POST" action="{{ route('register') }}">
                    ...
                    <x-sistem.forms.input-form id="name" type="text" placeholder="{{ __('Nombre') }}" :value="old('name')" name="name" wire:model="name"
                    autofocus />
                    ...
                    <x-sistem.forms.input-form id="password_confirmation" type="password_confirmation" placeholder="{{ __('************') }}" wire:model="password_confirmation" name="password_confirmation"
                     />
    
                    <div class="flex mt-6 text-sm">
                    <label for="policy" class="flex items-center">
                      <x-sistem.forms.checkbox-form id="policy" name="policy" wire:model="policy" />
                      <span class="ml-2 text-sm text-gray-600">{{ __('Acepto las politicas de privacidad') }}</span>
                    </label>
                    </div>
    
                    <!-- You should use a button here, as the anchor is only used for the example  -->
                    <x-sistem.buttons.primary-btn type="submit" class="w-full mt-4" wire:loading.attr="disabled" title="Registrarse"/>
